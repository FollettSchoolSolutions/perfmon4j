...
- `hawtio-plugin/` — Hawtio console plugin for authoring perfmon4j `mBeanSnapshotMonitor` config from a live JMX MBean. TypeScript/React/npm project, **not** a Maven module — like `stress-test/`, it is not listed in root `pom.xml`'s `<modules>` and has its own independent build tooling.
