...
public class TransformerParams {
    ...
    static {
    	String[] blackListElements = {
			...
			"org.jboss.bootstrap",
			"org.jboss.logging",
			"P(\\$\\$\\$view\\d+$)",  // Problem with classes generated by jboss 7.x org.jboss.invocation.proxy.AbstractProxyFactory
			"P(\\$Proxy\\$_\\$\\$)",  // Problem with proxy classes generated by weld.
		};
    	BLACKLIST.addAll(Arrays.asList(blackListElements));
    }
    ...
    int getTransformMode(String className) {
        ...
        if (mode != MODE_NONE) {
        	int len = getMatchElement(BLACKLIST, className).matchLength;
    		if (len > 0) {
    ...
    private static final Pattern REGEX_BASED_ELEMENT = Pattern.compile("P\\((.*)\\)");
    private final Map<String, Pattern> patternCache = Collections.synchronizedMap(new HashMap<String, Pattern>());
    private static final Pattern INVALID_PATTERN = Pattern.compile("SINGLETON USED TO CACHE INVALID PATTERN");
    
    private Pattern getUserPattern(String regex) {
    	Pattern result = null;
    	
    	result = patternCache.get(regex);
    	if (result == null) {
    		try {
    			result = Pattern.compile(regex);
    		} catch (PatternSyntaxException pse) {
    			logger.logError("Invalid pattern found will be ignored: " + regex);
    			result = INVALID_PATTERN;
    		}
    		patternCache.put(regex, result);
    	}
    	
    	return INVALID_PATTERN.equals(result) ? null : result;
    }
    ...
    private MatchElement getMatchElement(List<?> list, String className) {
        ...
        for (int i = 0; i < listSize; i++) {
            ...
            // Check to see if this is a Regular expression based element..
            Matcher matcher = REGEX_BASED_ELEMENT.matcher(element);
            if (matcher.matches()) {
            	Pattern userPattern = getUserPattern(matcher.group(1));
            	if (userPattern != null) {
            		Matcher userPatternMatcher = userPattern.matcher(className);
            		if (userPatternMatcher.find()) {
	                	matchElement = new MatchElement(className.length(), list.get(i));
            		}
            	}
            } else {	
