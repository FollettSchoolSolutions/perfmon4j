...
public class GenericFilterTest {
	...
	@Test
	void testMaskPasswordInQueryString() {
		assertEquals("?password=*******", GenericFilter.maskPassword("?password=dave"), "Password only parameter");
		assertEquals("?user=dave&password=*******", GenericFilter.maskPassword("?user=dave&password=dave"), "Password second parameter");
		assertEquals("?user=dave&password=*******&time=now", GenericFilter.maskPassword("?user=dave&password=dave&time=now"), "Password middle parameter");
		assertEquals("?password=*******&password=*******&password=*******&password=*******", GenericFilter.maskPassword("?password=dave&password=this is a test&password=&password=t"), "Multiple password parameters");
		assertEquals("?word=password", GenericFilter.maskPassword("?word=password"), "Password in value is ignored");
		assertEquals("?password=*******;x=y", GenericFilter.maskPassword("?password=dave;x=y"), "Work with ; separator");
	}

	@Test
	void testMaskPasswordWithMixedCaseInQueryString() {
		assertEquals("?Password=*******", GenericFilter.maskPassword("?Password=dave"), "Password only parameter");
	}

	@Test
	void testMaskParameterEndingWithPassword() {
		assertEquals("?userPassword=*******", GenericFilter.maskPassword("?userPassword=dave"), "Parameter ending with password");
		assertEquals("?user=dave&userPassword=*******", GenericFilter.maskPassword("?user=dave&userPassword=dave"), "Second parameter ending with password");
		assertEquals("?userPassword=*******&time=now", GenericFilter.maskPassword("?userPassword=dave&time=now"), "Middle parameter ending with password");
		assertEquals("?UserPassword=*******", GenericFilter.maskPassword("?UserPassword=dave"), "Mixed case parameter ending with password");
...
//System.out.println(result.toAppenderString());
			assertNotNull(result, "Did not recieve expected appender data outout of type: " + IntervalData.class);
	...
	@Test
	void testDefaultFilterParmas() {
		FilterParams params = FilterParams.getDefault();
		assertEquals("WebRequest", params.getBaseFilterCategory(), "baseFilterCategory");
		assertNull(params.getAbortTimerOnURLPattern(), "abortTimerOnURLPattern");
		assertNull(params.getSkipTimerOnURLPattern(), "skipTimerOnURLPattern");
		assertNull(params.getServletPathTransformationPattern(), "servletPathTransformationPattern");

		assertTrue(params.isAbortTimerOnImageResponse(), "abortTimerOnImageResponse");
		assertTrue(params.isAbortTimerOnRedirect(), "abortTimerOnRedirect");
		assertTrue(params.isOutputRequestAndDuration(), "outputRequestAndDuration");
	}

	@Test
	void testParseFilterParmas() {
		...
		assertEquals("RestRequest", params.getBaseFilterCategory(), "baseFilterCategory");
		assertEquals("/dont-time", params.getAbortTimerOnURLPattern(), "abortTimerOnURLPattern");
		assertEquals("/skip", params.getSkipTimerOnURLPattern(), "skipTimerOnURLPattern");
		assertEquals("/rest/context/*/ => /", params.getServletPathTransformationPattern(), "servletPathTransformationPattern");

		assertFalse(params.isAbortTimerOnImageResponse(), "abortTimerOnImageResponse");
		assertFalse(params.isAbortTimerOnRedirect(), "abortTimerOnRedirect");
		assertFalse(params.isOutputRequestAndDuration(), "outputRequestAndDuration");
	}

	@Test
	void testSimpleMonitor() throws Exception {
		...
		assertEquals(1, data.getTotalHits(), "Should of had 1 hit");
	}
	...
	@Test
	void testAbortTimerOnImageResponse() throws Exception {
		...
		assertEquals(1, data.getTotalHits(), "Should of had 1 hit");
		assertEquals(0, data.getTotalCompletions(), "Completion should have been aborted");
	}

	@Test
	void testAbortTimerOnRedirect() throws Exception {
		FilterParamsVO params = new FilterParamsVO();
	...
	@Test
	void testAbortTimerOnURLPattern() throws Exception {
		FilterParamsVO params = new FilterParamsVO();
		...
		assertEquals(2, data.getTotalHits(), "Both should have hit");
		assertEquals(1, data.getTotalCompletions(), "Only the request that did not match pattern should have been flagged complete");
	}

	@Test
	void testServletPathTransformationPattern() throws Exception {
		FilterParamsVO params = new FilterParamsVO();
		...
		assertEquals(1, data.getTotalCompletions(), "Should have been captured in WebRequest.rest category");
	}

	@Test
	void testSkipTimerOnURLPattern() throws Exception {
		FilterParamsVO params = new FilterParamsVO();
		...
		assertEquals(1, data.getTotalHits(), "Only ther request that did not match pattern should have been flagged as a hit");
		assertEquals(1, data.getTotalCompletions(), "Only the request that did not match pattern should have been flagged complete");
	}
