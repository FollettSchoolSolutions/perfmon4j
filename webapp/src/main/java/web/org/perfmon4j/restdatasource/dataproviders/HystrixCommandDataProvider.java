...
public class HystrixCommandDataProvider extends DataProvider {
	private static final String TEMPLATE_NAME = "HystrixCommand";
	private final HystrixCommandTemplate categoryTemplate;
	private static final Logger logger = LoggerFactory.initLogger(HystrixCommandDataProvider.class);
	private static final double REQUIRED_DB_VERSION = 7.0;
	
	public HystrixCommandDataProvider() {
		super(TEMPLATE_NAME);
		categoryTemplate = new HystrixCommandTemplate(TEMPLATE_NAME);
	}
	...
	public void processResults(Connection conn, RegisteredDatabaseConnections.Database db, ResultAccumulator accumulator, SeriesField[] fields, long start, 
		...
		if (JDBCHelper.getDatabaseVersion(conn, db.getSchema()) >= REQUIRED_DB_VERSION) { 
			...
			String query =
				...
				+ "FROM " + schemaPrefix + "P4JHystrixCommand pid\r\n"
		...
		if (JDBCHelper.getDatabaseVersion(conn, database.getSchema()) >= REQUIRED_DB_VERSION) {  
			...
			try {
				...
				String SQL = "SELECT SystemID, SystemName "
					...
					+ " FROM " + schema + "P4JHystrixCommand pid WHERE pid.SystemID = s.SystemID "
		...
		if (JDBCHelper.getDatabaseVersion(conn, db.getSchema()) >= REQUIRED_DB_VERSION) {  
			...
			try {
				...
				String SQL = "SELECT DISTINCT(KeyName) "
					+ " FROM " + schema + "P4JHystrixCommand pid "
	...
	private static class HystrixCommandTemplate extends CategoryTemplate {

		private HystrixCommandTemplate(String templateName) {
		...
		private static final Field[] buildFields() {
			...
			fields.add(new NaturalPerMinuteProviderField("semaphoreRejectedCount","systemID", "startTime", "endTime", "SEMAPHOREREJECTEDCOUNT"));
			fields.add(new NaturalPerMinuteProviderField("threadPoolRejectedCount","systemID", "startTime", "endTime", "THREADPOOLREJECTEDCOUNT"));
			fields.add(new NaturalPerMinuteProviderField("shortCircuitedCount","systemID", "startTime", "endTime", "SHORTCIRCUITEDCOUNT"));
			fields.add(new NaturalPerMinuteProviderField("timeoutCount","systemID", "startTime", "endTime", "TIMEOUTCOUNT"));
			fields.add(new NaturalPerMinuteProviderField("failureCount","systemID", "startTime", "endTime", "FAILURECOUNT"));
			fields.add(new NaturalPerMinuteProviderField("successCount","systemID", "startTime", "endTime", "SUCCESSCOUNT").makePrimary());
