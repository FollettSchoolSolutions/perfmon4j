...
public class CacheDataProviderTest extends TestCase {

	private static String DATABASE_NAME = "Production";
	private final BaseDatabaseSetup databaseSetup = new BaseDatabaseSetup();
	private final CacheDataProvider provider = new CacheDataProvider();
	...
	public CacheDataProviderTest(String name) {
		super(name);
	}
	...
	public void testSplitSubCategory() {
		String[] result;
		
		result = provider.splitSubCategory("MongoCache.ProviderResult");
		assertEquals("MongoCache", result[0]);
		assertEquals("ProviderResult", result[1]);

		result = provider.splitSubCategory(".ProviderResult");
		assertEquals("", result[0]);
		assertEquals("ProviderResult", result[1]);
		
		result = provider.splitSubCategory("MongoCache");
		assertEquals("MongoCache", result[0]);
		...
		result = provider.splitSubCategory(".");
		assertEquals("", result[0]);
		assertEquals("", result[1]);
		...
		result = provider.splitSubCategory(null);
		assertEquals("", result[0]);
		assertEquals("", result[1]);
	}
	...
	public void testLookupMonitoredSystems() throws Exception {
		...
		try {
			...
			// Add a Observation...
			databaseSetup.addCacheObservation(1, System.currentTimeMillis(), "MongoCache", "Provider");
	...
	public void testLookupMonitoredCategories() throws Exception {
		...
		try {
			...
			databaseSetup.addCacheObservation(1, System.currentTimeMillis(), "Mongo", "Provider");
			...
			assertEquals("Cache.Mongo.Provider", cat.getName());
			assertEquals("Cache", cat.getTemplateName());
			...
			databaseSetup.addCacheObservation(1, System.currentTimeMillis(), "Infinispan", "Provider");
			result = provider.lookupMonitoredCategories(conn, database, systems, getStartTime(), getEndTime());
			assertNotNull(result);
	...
	public void testQuery() throws Exception{
		...
		try {
			...
			String seriesDef = 
				TestHelper.buildSeriesDefinitionWithAllFields(getDefaultSystemID(), "Cache.Mongo.Provider", 
