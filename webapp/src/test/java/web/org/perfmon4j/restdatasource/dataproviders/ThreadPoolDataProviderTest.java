...
public class ThreadPoolDataProviderTest extends TestCase {

	private static String DATABASE_NAME = "Production";
	private final BaseDatabaseSetup databaseSetup = new BaseDatabaseSetup();
	private final ThreadPoolDataProvider provider = new ThreadPoolDataProvider();
	...
	public ThreadPoolDataProviderTest(String name) {
		super(name);
	}
	...
	public void testLookupMonitoredSystems() throws Exception {
		...
		try {
			...
			// Add a Observation...
			databaseSetup.addThreadPoolObservation(1, System.currentTimeMillis(), "JBoss", "ajp-thread-pool");
	...
	public void testLookupMonitoredCategories() throws Exception {
		...
		try {
			...
			assertEquals("ThreadPool.JBoss.ajp-thread-pool", cat.getName());
			assertEquals("ThreadPool", cat.getTemplateName());
			...
			databaseSetup.addThreadPoolObservation(1, System.currentTimeMillis(), "MetaSearch", "search-workers");
			result = provider.lookupMonitoredCategories(conn, database, systems, getStartTime(), getEndTime());
			assertNotNull(result);
	...
	public void testQuery() throws Exception{
		...
		try {
			...
			String seriesDef = 
				TestHelper.buildSeriesDefinitionWithAllFields(getDefaultSystemID(), "ThreadPool.JBoss.ajp-thread-pool", 
