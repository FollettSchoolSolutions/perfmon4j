...
public class HystrixCommandDataProviderTest extends TestCase {
	private static String DATABASE_NAME = "Production";
	private final BaseDatabaseSetup databaseSetup = new BaseDatabaseSetup();
	private final HystrixCommandDataProvider provider = new HystrixCommandDataProvider();
	...
	public HystrixCommandDataProviderTest(String name) {
		super(name);
	}
	...
	public void testLookupMonitoredSystems() throws Exception {
		...
		try {
			...
			assertEquals("Should have no systems with HystrixCommand observations", 0, result.size());
			
			// Add an Observation...
			databaseSetup.addHystrixCommandObservation(1, System.currentTimeMillis(), "default");
	...
	public void testLookupMonitoredCategories() throws Exception {
		...
		try {
			...
			assertEquals("HystrixCommand.default", cat.getName());
			assertEquals("HystrixCommand", cat.getTemplateName());
			...
			databaseSetup.addHystrixCommandObservation(1, System.currentTimeMillis(), "not default");
	...
	public void testQuery() throws Exception{
		...
		try {
			...
			String seriesDef = 
				TestHelper.buildSeriesDefinitionWithAllFields(getDefaultSystemID(), "HystrixCommand.default", 
					provider.getCategoryTemplate());
					
			databaseSetup.addHystrixCommandObservation(1, System.currentTimeMillis(), "default");
			
//System.out.println(JDBCHelper.dumpQuery(conn, "SELECT * FROM P4JHystrixKey"));
//System.out.println(JDBCHelper.dumpQuery(conn, "SELECT * FROM P4JHystrixCommand"));
